...
using ThunderRunners.GoalOrientedBehaviour.Scripts.GameData.Actions;
...
namespace ThunderRunners
{
    public class TeamManager : MonoBehaviour
    {
        ...
        public static Vector3 defendingPosition = new Vector3(0, 0, 0);
        ...
        public static float throwFlagCheckSurroundingsRadius = 10f;
        ...
        public static float throwFlagAngleToEnemyCheck = 6f;
        ...
        public float defenseTackleRadius = 1.5f;
        ...
        [Tooltip("Backup readonly variable.")]
        public static readonly float minimumVelocityBeforeThrowingFlag_backup = 4.5f;
        ...
        [Tooltip("Number of nodes to check for each side of the flag's node (square with flag in the centre and 'stuckFlagPosCheckDepth * 2' side length.")]
        public static int stuckFlagPosCheckDepth = 5;
        ...
        // List of runners that completed the run to stuck flag action since the flag got stuck
        public static List<Runner> runnersThatRanToStuckFlag;

        public Grid grid;

        public static bool isFlagStuck = false;
        ...
        private void Awake()
        {
            ...
            // Get our quadrant
            if(_myTeamBase.transform.position.x < 0)
            {
                teamQuadrant.x = -1.5f; teamQuadrant.z = -1.5f;
                enemyQuadrant.x = 0.8f; enemyQuadrant.z = 1.6f;
                defendingPosition = new Vector3(23f, 0, 23f);
            }
            else if (_myTeamBase.transform.position.x > 0)
            {
            ...
            // List of runners that completed the run to stuck flag action since the flag got stuck
            runnersThatRanToStuckFlag = new List<Runner>();
        ...
        private IEnumerator RequestNewPlan()
        {
            while (true)
            {
                ...
                {
                    if (agent.NeedNewPlan && Vector3.Distance(agent.GetComponent<Runner>().transform.position, _flag.transform.position) > 1f)
        ...
        private IEnumerator AdaptActionsCosts()
        {
            while (true)
            {
                foreach (var runner in MyRunners)
                {
                    ...
                    if (isFlagPosWalkable())
                        runnersThatRanToStuckFlag.Clear();

                    // If the flag is stuck
                    // runner closest to the flag - attack the flag
                    // From the remaining runners, the closest to middle - goes to middle
                    // the remaining runner - attack the flag
                    if (!isFlagPosWalkable())
                    {
                        // List of the runners that aren't the closest to the flag
                        List<Runner> remainingteammates = new List<Runner>() { teamByDistFlag[1], teamByDistFlag[2] };

                        // Order them by their distance to middle
                        List<Runner> teamByDistMiddle = getTeammatesByDistanceToTarget(remainingteammates, new Vector3(0, 0, 0));

                        // If this runner (closest to flag) isn't running middle and hasn't completed the RunToStuckFlag action yet then update his costs
                        if (teamByDistFlag[0].GetComponent<RunToMiddle>().Cost != 0 && !runnersThatRanToStuckFlag.Contains(teamByDistFlag[0]))
                        {
                            // Run to stuck flag
                            teamByDistFlag[0].GetComponent<RunToStuckFlag>().Cost = 0;
                        ...
                        // If there isn't anyone already either going or in middle
                        if (!isAnyTeamMemberGoingMiddle(teamByDistMiddle[0]))
                        {
                            // Run to middle
                            teamByDistMiddle[0].GetComponent<RunToStuckFlag>().Cost = float.PositiveInfinity;
                            teamByDistMiddle[0].GetComponent<RunToMiddle>().Cost = 0;
                            teamByDistMiddle[0].GetComponent<RunToDefense>().Cost = float.PositiveInfinity;
                        }

                        // If this runner (remaining) isn't running middle and hasn't completed the RunToStuckFlag action yet then update his costs
                        if (teamByDistMiddle[1].GetComponent<RunToMiddle>().Cost != 0 && !runnersThatRanToStuckFlag.Contains(teamByDistMiddle[1]))
                        {
                            // Run to stuck flag
                            teamByDistMiddle[1].GetComponent<RunToStuckFlag>().Cost = 0;
                            teamByDistMiddle[1].GetComponent<RunToMiddle>().Cost = float.PositiveInfinity;
                            teamByDistMiddle[1].GetComponent<RunToDefense>().Cost = float.PositiveInfinity;
                        }
                    }
                    ...
                    else if (isFlagInEnemyQuadrant())
                    {
                        //runner.GetComponent<RunToStuckFlag>().Cost = float.PositiveInfinity;

                        teamByDistFlag[0].GetComponent<RunToMiddle>().Cost = float.PositiveInfinity;
                        teamByDistFlag[0].GetComponent<RunToDefense>().Cost = float.PositiveInfinity;
                        teamByDistFlag[0].GetComponent<RunToStuckFlag>().Cost = float.PositiveInfinity;
                    ...
                    // If the game is in normal conditions, we want to pickup the flag
                    else
                    {
                        runner.GetComponent<RunToMiddle>().Cost = float.PositiveInfinity;
                        runner.GetComponent<RunToDefense>().Cost = float.PositiveInfinity;
                    ...
                    if (isTeamPointGuaranteed(runner) && isFlagInTeamQuadrant() && isFlagPosWalkable())
        ...
        public static List<Runner> getTeammatesByDistanceToTarget(List<Runner> runners, Vector3 target)
        {
        ...
        // Updates the flag's status
        public void updateIsFlagStuck()
        {
            isFlagStuck = !grid.NodeFromWorldPoint(_flag.transform.position).Walkable;
        }

        // Returns whether or not the flag is stuck
        public static bool isFlagPosWalkable()
        {
            return !isFlagStuck;
        }
    }
